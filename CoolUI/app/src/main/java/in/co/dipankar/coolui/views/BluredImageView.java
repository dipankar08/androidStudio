...
public class BluredImageView extends AppCompatImageView {
  ...
  protected void onDraw(Canvas canvas) {
    ...
    if (blurPaint == null) {
      originalBitmap = Bitmap.createBitmap(getWidth(), getHeight(), Bitmap.Config.ARGB_4444);
  ...
  /**
   * Generate blurred image using RenderScript or alternative implementation (if RS is not
   * supported)
   *
  ...
  /**
   * Set blur level. Do not use this method repeatedly on user generated events (like inside
   * onScrollChanged) because this method is heavy (causes new bitmap generation).
   *
  ...
  /**
   * Use this function to smoothly change from sharp image to blurred image. Can be used for example
   * inside onScrollChanged or similar
   *
  ...
  public void setFocus(float focus) {
    ...
    if (resizeMatrix != null) {
      ...
      resizeMatrix.postTranslate(
          -0.5f * getWidth() * validFocus / 10.0f, -0.5f * getHeight() * validFocus / 10.0f);
    }
