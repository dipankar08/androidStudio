...
class Camera1Session implements CameraSession {
   ...
   // private static final Histogram camera1StopTimeMsHistogram = Histogram.createCounts("WebRTC.Android.Camera1.StopTimeMs", 1, 10000, 50);
    ...
    private Camera1Session(Events events, boolean captureToTexture, Context applicationContext, SurfaceTextureHelper surfaceTextureHelper, @Nullable MediaRecorder mediaRecorder, int cameraId, Camera camera, CameraInfo info, CaptureFormat captureFormat, long constructionTimeNs) {
        ...
        this.surfaceTextureHelper.startListening(new OnTextureFrameAvailableListener() {
            public void onTextureFrameAvailable(int oesTextureId, float[] transformMatrix, long timestampNs) {
                ...
                if(Camera1Session.this.state != Camera1Session.SessionState.RUNNING) {
                    ...
                    Buffer buffer = Camera1Session.this.surfaceTextureHelper.createTextureBuffer(Camera1Session.this.captureFormat.width, Camera1Session.this.captureFormat.height, RendererCommon.convertMatrixToAndroidGraphicsMatrix(transformMatrix));
