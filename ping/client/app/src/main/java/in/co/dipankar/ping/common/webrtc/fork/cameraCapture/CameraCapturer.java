...
import in.co.dipankar.ping.common.webrtc.fork.cameraCapture.CameraSession.FailureType;
...
abstract class CameraCapturer implements CameraVideoCapturer {
  ...
  @Nullable
  private final CreateSessionCallback createSessionCallback =
      new CreateSessionCallback() {
        public void onDone(CameraSession session) {
          CameraCapturer.this.checkIsOnCameraThread();
          Logging.d(
              "CameraCapturer",
              "Create session done. Switch state: "
                  + CameraCapturer.this.switchState
                  + ". MediaRecorder state: "
                  + CameraCapturer.this.mediaRecorderState);
          CameraCapturer.this.uiThreadHandler.removeCallbacks(
              CameraCapturer.this.openCameraTimeoutRunnable);
          synchronized (CameraCapturer.this.stateLock) {
            CameraCapturer.this.capturerObserver.onCapturerStarted(true);
            CameraCapturer.this.sessionOpening = false;
            CameraCapturer.this.currentSession = session;
            CameraCapturer.this.cameraStatistics =
                new CameraStatistics(
                    CameraCapturer.this.surfaceHelper, CameraCapturer.this.eventsHandler);
            CameraCapturer.this.firstFrameObserved = false;
            CameraCapturer.this.stateLock.notifyAll();
            if (CameraCapturer.this.switchState == CameraCapturer.SwitchState.IN_PROGRESS) {
              if (CameraCapturer.this.switchEventsHandler != null) {
                CameraCapturer.this.switchEventsHandler.onCameraSwitchDone(
                    CameraCapturer.this.cameraEnumerator.isFrontFacing(
                        CameraCapturer.this.cameraName));
            ...
            if (CameraCapturer.this.mediaRecorderState
                    == CameraCapturer.MediaRecorderState.IDLE_TO_ACTIVE
                || CameraCapturer.this.mediaRecorderState
                    == CameraCapturer.MediaRecorderState.ACTIVE_TO_IDLE) {
        ...
        public void onFailure(FailureType failureType, String error) {
          ...
          synchronized (CameraCapturer.this.stateLock) {
            ...
            if (CameraCapturer.this.openAttemptsRemaining <= 0) {
              ...
              if (CameraCapturer.this.mediaRecorderState
                  != CameraCapturer.MediaRecorderState.IDLE) {
                if (CameraCapturer.this.mediaRecorderEventsHandler != null) {
  ...
  @Nullable
  private final Events cameraSessionEventsHandler =
      new Events() {
  ...
  private void createSessionInternal(int delayMs, final MediaRecorder mediaRecorder) {
    ...
    this.cameraThreadHandler.postDelayed(
        new Runnable() {
          public void run() {
            CameraCapturer.this.createCameraSession(
                CameraCapturer.this.createSessionCallback,
                CameraCapturer.this.cameraSessionEventsHandler,
                CameraCapturer.this.applicationContext,
                CameraCapturer.this.surfaceHelper,
                mediaRecorder,
                CameraCapturer.this.cameraName,
                CameraCapturer.this.width,
                CameraCapturer.this.height,
                CameraCapturer.this.framerate);
          }
        },
        (long) delayMs);
  }
    ...
    synchronized (this.stateLock) {
      while (this.sessionOpening) {
        ...
        try {
          ...
          Logging.w(
              "CameraCapturer", "Stop capture interrupted while waiting for the session to open.");
  ...
  public void addMediaRecorderToCamera(
    ...
    this.cameraThreadHandler.post(
        new Runnable() {
          public void run() {
            CameraCapturer.this.updateMediaRecorderInternal(
                mediaRecorder, mediaRecoderEventsHandler);
          }
        });
  }
  ...
  public void removeMediaRecorderFromCamera(final MediaRecorderHandler mediaRecoderEventsHandler) {
    ...
    this.cameraThreadHandler.post(
        new Runnable() {
          public void run() {
            CameraCapturer.this.updateMediaRecorderInternal(
                (MediaRecorder) null, mediaRecoderEventsHandler);
          }
        });
  }
    ...
    if (deviceNames.length < 2) {
      ...
      synchronized (this.stateLock) {
        ...
        if (this.mediaRecorderState != CameraCapturer.MediaRecorderState.IDLE) {
          this.reportCameraSwitchError(
              "switchCamera: media recording is active", switchEventsHandler);
          return;
        }
  ...
  private void updateMediaRecorderInternal(
    ...
    Logging.d(
        "CameraCapturer",
        "updateMediaRecoderInternal internal. State: "
    ...
    Object var4 = this.stateLock;
    synchronized (this.stateLock) {
      if (addMediaRecorder && this.mediaRecorderState != CameraCapturer.MediaRecorderState.IDLE
          || !addMediaRecorder
              && this.mediaRecorderState != CameraCapturer.MediaRecorderState.ACTIVE) {
        this.reportUpdateMediaRecorderError(
  ...
  protected abstract void createCameraSession(
      CreateSessionCallback var1,
      Events var2,
      Context var3,
      SurfaceTextureHelper var4,
      MediaRecorder var5,
      String var6,
      int var7,
      int var8,
