...
class Camera1Session implements CameraSession {
  ...
  private static CaptureFormat findClosestCaptureFormat(
      Parameters parameters, int width, int height, int framerate) {
    List<FramerateRange> supportedFramerates =
  ...
  private void listenForTextureFrames() {
    this.surfaceTextureHelper.startListening(
        new OnTextureFrameAvailableListener() {
          public void onTextureFrameAvailable(
              int oesTextureId, float[] transformMatrix, long timestampNs) {
            Camera1Session.this.checkIsOnCameraThread();
            if (Camera1Session.this.state != Camera1Session.SessionState.RUNNING) {
              Logging.d(
                  "Camera1Session", "Texture frame captured but camera is no longer running.");
                ...
                rotation =
                    (int)
                        TimeUnit.NANOSECONDS.toMillis(
                            System.nanoTime() - Camera1Session.this.constructionTimeNs);
              ...
              Buffer buffer =
                  Camera1Session.this.surfaceTextureHelper.createTextureBuffer(
  ...
  private void listenForBytebufferFrames() {
    this.camera.setPreviewCallbackWithBuffer(
        new PreviewCallback() {
          public void onPreviewFrame(byte[] data, Camera callbackCamera) {
            Camera1Session.this.checkIsOnCameraThread();
            if (callbackCamera != Camera1Session.this.camera) {
              Logging.e(
                  "Camera1Session", "Callback from a different camera. This should never happen.");
            } else if (Camera1Session.this.state != Camera1Session.SessionState.RUNNING) {
              Logging.d(
                  "Camera1Session", "Bytebuffer frame captured but camera is no longer running.");
            } else {
                  ...
                  new NV21Buffer(
                      ...
                      () -> {
                        Camera1Session.this.cameraThreadHandler.post(
                            () -> {
                              if (Camera1Session.this.state
                                  == Camera1Session.SessionState.RUNNING) {
              ...
              VideoFrame frame =
                  new VideoFrame(
                      frameBuffer, Camera1Session.this.getFrameOrientation(), captureTimeNs);
              Camera1Session.this.events.onFrameCaptured(Camera1Session.this, frame);
              frame.release();
            }
          }
  ...
  static {
    // camera1ResolutionHistogram = Histogram.createEnumeration("WebRTC.Android.Camera1.Resolution",
