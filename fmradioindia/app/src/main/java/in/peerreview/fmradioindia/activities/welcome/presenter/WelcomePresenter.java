...
public class WelcomePresenter implements IWelcomeContract.Presenter{
    final static String TAG = "WelcomePresenter";
    ...
    public void loadData(){
        ...
        network.retrive(url, Network.CacheControl.GET_LIVE_ELSE_CACHE, new Network.INetworkCallback() {
            @Override
                    ...
                    for (int i = 0; i < Jarray.length(); i++) {
                        ...
                        if(object.has("name") && object.has("url")) { //TODO
                            nodes.add(new RadioNode(
                                    ...
                                    object.optInt("count_click",0), RadioNode.TYPE.SONG));

                        }
                    }

                    Collections.sort(nodes, new Comparator<RadioNode>() {
                        @Override
