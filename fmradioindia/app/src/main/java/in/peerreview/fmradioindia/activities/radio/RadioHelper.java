...
public class RadioHelper {
    ...
    public void shouldShowRateDialog(boolean force){
        if(mSharedPrefsUtil.getBoolean(PREF_FLAG_RATE_DIALOG, Boolean.TRUE) || force){
            ...
            DialogUtils.showPlayStoreRateAlert(mContext, new DialogUtils.PlayStoreRateDialogCallback() {
                @Override
    ...
    public void shouldShowUpgradeDialog() {
        ...
        final String gkString = FMRadioIndiaApplication.Get().getGateKeeperUtils().getRemoteSetting(GK_VERSION_STR, null);
        if(gkString == null){
            return;
        }
        ...
        FMRadioIndiaApplication.Get().getTelemetry().markHit(Constants.TELEMETRY_SHOW_MENU_UPGRADE_AUTO);
        DialogUtils.showAppUpgradeAlert(mContext, new DialogUtils.AppUpgradeCallback() {
            @Override
            public void onSubmit() { }
