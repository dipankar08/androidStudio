...
public class FullScreenPlayerView extends BaseView
    implements FullScreenPlayerViewPresenter.ViewContract {
    ...
    mBack.setOnClickListener(
        new OnClickListener() {
          ...
          public void onClick(View v) {
            getNavigation().goBackAfterShowingAds();
          }
        });
        ...
        new OnClickListener() {
          ...
          public void onClick(View view) {
            FmRadioApplication.Get()
                .getRadioManager()
                .toggleLove(
                    FmRadioApplication.Get().getRadioManager().getCurrentID(),
                    new RadioManager.LoveCallback() {
                      @Override
                      public void isLoveMarked(boolean isMarked) {
                        feb.setImageResource(
                            isMarked
  ...
  protected void onAttachedToWindow() {
    ...
    refershUI(
        FmRadioApplication.Get().getRadioManager().getCurrentRadio(),
        FmRadioApplication.Get().getRadioManager().getState());
    if (getArgs() == null) {
      return;
    }
    ...
    feb.setImageResource(
        FmRadioApplication.Get().getRadioManager().isLove(id)
            ? R.drawable.ic_love_fill_white_32
            : R.drawable.ic_love_white_32);
  }
  ...
  private void refershUI(Radio mRadio, RadioManager.STATE state) {
    ...
    Glide.with(getContext()).load(mRadio.getImageUrl()).into(mImage);
    ...
    feb.setImageResource(
        FmRadioApplication.Get().getRadioManager().isLove(mRadio.getId())
            ? R.drawable.ic_love_fill_white_32
            : R.drawable.ic_love_white_32);
    ...
    switch (state) {
      ...
      case ERROR:
        Toast.makeText(
                getContext(),
                "Not able to play" + mRadio.getName() + ", please retry.",
                Toast.LENGTH_SHORT)
            .show();
    }
  }
